...
out_file <- file.path(results_path, "cLS-MDYPL-comparison-estimates.rda")
n_cores <- 10
...
mdypl <- function(X, y, alpha, start = NULL, se_start = c(0.5, 1, 1)) {
    ...
    ss <- try(summary(fit, hd_correction = TRUE,  se_start = se_start, init_iter = 0,
                      se_solve_dots = list(ftol = 1e-12, xtol = 1e-12)), silent = TRUE)
    if (inherits(ss, "try-error")) { # try another time
        ss <- summary(fit, hd_correction = TRUE,  se_start = se_start * 0.9, init_iter = 0,
                      se_solve_dots = list(ftol = 1e-12, xtol = 1e-12))
    }
...
if (file.exists(out_file)) {
    ...
    nk <- nk |> mutate(root_aMSE = sqrt(2 / (n + 1 / kappa)) * sigma / mu)
    ...
    estimates <- z_statistics <- constants <- NULL
    for (i in 1:n_settings) {
        n <- nk[i, "n"]
        kappa <- nk[i, "kappa"]
        b <- nk[i, "b"]
        ...
        constants <- rbind(constants,
                           do.call("rbind", consts) |> mutate(sample = 1:R))
        estimates <- rbind(estimates,
                           data.frame(estimate = c(beta_cls, beta_cls_oracle, beta_mdypl, beta_mdypl_oracle),
                                      method = rep(methds, each = R * p),
                                      truth = beta_oracle,
                                      parameter = rep(1:p, R * n_methds),
                                      sample = rep(rep(1:R, each = p), n_methds),
                                      kappa = kappa, gamma = gamma, N = n, p = p))
        z_statistics <- rbind(z_statistics,
                              data.frame(z = c(z_cls, z_cls_oracle, z_mdypl, z_mdypl_oracle),
    ...
    save(nk, constants, estimates, z_statistics, constants, file = out_file)
}
...
aBias <- estimates |>
    ...
    group_by(method, N, kappa) |> summarize(aBias = mean(aBiases),
                                            ...
                                            R = n())
aBias_zero <- estimates |> filter(truth == 0) |>
    group_by(method, sample, N, kappa) |> summarize(aBiases = mean(estimate - truth)) |>
    group_by(method, N, kappa) |> summarize(aBias = mean(aBiases),
                                            sd = sd(aBiases) / sqrt(n()),
                                            R = n())
aBias_nonzero <- estimates |> filter(truth != 0) |>
    group_by(method, sample, N, kappa) |> summarize(aBiases = mean(estimate - truth)) |>
    group_by(method, N, kappa) |> summarize(aBias = mean(aBiases),
                                            sd = sd(aBiases) / sqrt(n()),
                                            R = n())
...
p_pvalue <- ggplot(z_statistics |> subset(parameter %in% which_par & N %in% which_N)) +
